...
function synthesizeAnswer(question, extracted, failedSources, allSources) {
    ...
    for (const src of extracted) {
        ...
        if (fragment)
            extractedFragments.push({ source: src.title, text: fragment });
    }
    ...
    for (const src of failedSources) {
        if (src.snippet) {
            ...
            if (looksLikeProse(t) || t.length >= 20)
                snippetFragments.push({ source: src.title, text: t });
        }
    }
    for (const src of allSources) {
        if (snippetFragments.some(f => f.source === src.title))
            continue;
        if (src.snippet) {
            const t = scrubFragmentText(src.snippet);
            if (looksLikeProse(t) || t.length >= 20)
