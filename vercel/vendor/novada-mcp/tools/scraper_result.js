...
export const ScraperResultParamsSchema = z.object({
    task_id: z
        ...
        .regex(/^[a-zA-Z0-9_\-\.]{1,128}$/, "task_id must be alphanumeric with underscores/hyphens/dots only")
...
export async function novadaScraperResult(params, apiKey) {
    ...
    try {
        ...
        if (Array.isArray(body) && body.length > 0) {
            ...
            if (bObj.code === 27202) {
                return JSON.stringify({
                    status: "not_ready",
                    task_id,
                    agent_instruction: "Task is not yet complete. Use novada_scraper_status to poll until status is 'complete', then call novada_scraper_result again.",
                }, null, 2);
            }
        ...
        if (downloadErr instanceof AxiosError) {
            ...
            throw makeNovadaError(NovadaErrorCode.API_DOWN, `Download endpoint error (HTTP ${status ?? "network"}): ${downloadErr.message}`);
        }
        else {
            // Non-Axios error (e.g. network failure) — re-throw via error system
            throw makeNovadaError(NovadaErrorCode.API_DOWN, downloadErr instanceof Error ? downloadErr.message : String(downloadErr));
        }
    }
    // ── No data retrieved ──
    if (rawData === null) {
