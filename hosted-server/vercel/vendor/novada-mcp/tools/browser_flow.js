...
import { z } from "zod";
import { classifyBusinessCode, classifyError, makeNovadaError, NovadaError, NovadaErrorCode, redactSecrets, sanitizeServerMsg, } from "../_core/errors.js";
    ...
    if (apiResponse.code !== 0) {
        ...
        if (apiResponse.code === 10001) {
            throw new NovadaError({
                ...
                agent_instruction: "Browser Flow returned code 10001 — its exact meaning is endpoint-specific and not documented. " +
        ...
        if (mapped !== NovadaErrorCode.UNKNOWN) {
            throw makeNovadaError(mapped, `Browser Flow API error (code ${apiResponse.code}): ${sanitizeServerMsg(msg)}`, `code ${apiResponse.code}`, apiResponse.code);
        }
        ...
        throw new NovadaError({
            ...
            agent_instruction: "The Browser Flow API returned an unrecognized error code. Verify the URL is publicly accessible " +
