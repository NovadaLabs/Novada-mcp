...
export async function submitScrapeTask(apiKey, scraper_name, scraper_id, params) {
    ...
    if (body.code !== 0) {
        ...
        if (mapped !== NovadaErrorCode.UNKNOWN) {
            throw makeNovadaError(mapped, `Scraper error (code ${body.code}): ${sanitizeServerMsg(msg)}`, `code ${body.code}`, body.code);
        }
        ...
        if (body !== null &&
            ...
            if (Number.isFinite(numericErrCode)) {
                ...
                if (mappedDownload !== NovadaErrorCode.UNKNOWN) {
                    throw makeNovadaError(mappedDownload, `Scraper download error (code ${numericErrCode}): ${sanitizeServerMsg(errMsg || "no upstream message")}`, `download_code:${numericErrCode}`, numericErrCode);
                }
            }
