      ...
      return {
        content: [{
          ...
          text: [
            ...
            "",
            `This hosted gateway allows ${monthlyQuota} free calls/month per API key (an anti-abuse cap, independent of your Novada balance). That monthly cap is now used up.`,
            "",
            ...
            "2. Make any real top-up/purchase at https://dashboard.novada.com/ — paid accounts are exempt from the gateway cap (takes effect within ~6 hours).",
            "3. Need a higher gateway cap? Contact sales@novada.com.",
            "",
            "agent_instruction: free_gateway_cap_reached | retry_recommended: false (retrying now will fail; after a real top-up/purchase the exemption applies within ~6 hours) | resets: start_of_next_month | alternatives: run local MCP via `npx novada-mcp@latest` (same key, no gateway cap) OR make a real top-up/purchase at https://dashboard.novada.com/ (paid accounts are exempt from the gateway cap) OR wait for monthly reset.",
          ].join("\n"),
        }],
        isError: true,
      };
